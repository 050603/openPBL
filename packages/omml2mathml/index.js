...
function setup (src, out, w) {
  if (w.res) return;
  // Reuse the standards-based DOM implementation that parsed the OMML input.
  // The upstream package depended on get-dom@9, which pulled jsdom@9 and the
  // retired request/form-data stack into browser builds. OMML conversion only
  // needs createDocument/createElementNS, both available on the input document.
  let sourceDoc = src.nodeType === 9 ? src : src.ownerDocument
    , doc = sourceDoc.implementation.createDocument(MATH_NS, null)
    , nod = nodal(doc, {}, nsMap)
