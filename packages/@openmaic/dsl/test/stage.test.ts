...
const quizContent: QuizContent = {
  ...
  questions: [
    {
      id: 'q1',
      knowledgePointIds: ['kp-arithmetic'],
      type: 'single',
...
describe('SceneContent (contract layer)', () => {
  it('is the universal slide/quiz compatibility subset', () => {
    ...
    expect(c2.questions[0].knowledgePointIds).toEqual(['kp-arithmetic']);
    expect(c2.questions[0].format).toBe('single_choice');
  });
});

describe('OpenPBL legacy media compatibility', () => {
  it('keeps legacy speech audioUrl readable during asset-ref migration', () => {
    const speech: SpeechAction = {
      id: 'speech-1',
      type: 'speech',
      text: 'Welcome',
      audioUrl: 'https://cdn.example.test/welcome.mp3',
    };
