...
要求：
1. 生成 6-10 个授课活动，覆盖课程引入、核心知识讲授、AI 授知、PBL 项目布置、小组推进、展示评价与反思。
2. 每个活动必须写清：
...
export function buildEvaluationPlanPrompt(input: GenerateInput, context?: { knowledgeGraph?: unknown; knowledgePoints?: unknown }): {
  ...
  const user = `请基于以下课程信息与已确认知识图谱，生成项目评价方案（4-6 个维度，权重合计 100%，含整体评价说明）。
...
3. overallRubric 说明教师评分、AI 参考评分和最终评分如何形成。
