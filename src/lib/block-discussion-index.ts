...
type BuildBlockDiscussionIndexOptions = {
  ...
  isTransientSuggestion: (node: TElement | TSuggestionText) => boolean;
};
...
export const buildBlockDiscussionIndex = ({
  ...
  entries.forEach(([node, path]) => {
    ...
    if (TextApi.isText(node)) {
      getCommentIds(node).forEach((commentId) => {
    ...
    if (isTransientSuggestion(node as TElement | TSuggestionText)) return;
...
const getDiscussionIndex = (
  ...
  const index = buildBlockDiscussionIndex({
    ...
    getCommentIds: (node) => getCommentKeys(node).map(getCommentKeyId),
    ...
    isTransientSuggestion: (node) => Boolean(
      (node as TElement | TSuggestionText & Record<string, unknown>)[
        getTransientSuggestionKey()
      ]
    ),
  });
