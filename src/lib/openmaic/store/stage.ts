...
import { createSelectors } from '@openmaic/lib/utils/create-selectors';
import type { ChatSession } from '@openmaic/lib/types/chat';
...
const useStageStoreBase = create<StageState>()((set, get) => ({
  ...
  setOutlines: (outlines) => {
    ...
    if (stageId) {
      ...
      import('@openmaic/lib/utils/database').then(({ db }) => {
  ...
  setGenerationComplete: (generationComplete) => {
    ...
    if (stageId) {
      ...
      void get()
        ...
        .then((saved) => {
          if (!saved) return;
          return import('@openmaic/lib/utils/database').then(({ db }) => {
            db.stageOutlines.put({
              stageId,
              outlines,
              generationComplete,
  ...
  saveToStorage: async () => {
    ...
    try {
      const { saveStageData } = await import('@openmaic/lib/utils/stage-storage');
  ...
  loadFromStorage: async (stageId: string) => {
    try {
      ...
      const { db } = await import('@openmaic/lib/utils/database');
