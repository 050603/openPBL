...
## Classroom Action Compatibility

{{snippet:widget-action-bridge}}

For 3D-specific state, expose `window.setWidgetState(state)` and map supported fields such as `cameraPosition`, `cameraTarget`, `scale`, `speed`, or selected object IDs to the real Three.js scene. Keep every `switch` case inside braces so block-scoped variables cannot collide. The generic DOM behavior in the bridge remains responsible for focus, annotations, and reveal controls.
