...
4. The outline is explicitly two-level: the confirmed first-level activity timeline is the parent schedule, and every generated outline below is a second-level resource detail. One first-level activity may have zero, one, or many details; details are not paired one-to-one with activities and must never use array position as a relationship.
5. Every detail must carry explicit `stageKey`, `stageLabel`, `audience`, `generationPurpose`, and `parentActivityId`. `parentActivityId` must equal the ID of a first-level activity supplied in the course facts. Keep `activityId` equal to the same first-level ID when the detail is an ordinary classroom support resource.
