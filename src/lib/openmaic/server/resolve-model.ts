...
import { getModel, parseModelString, type ModelWithInfo } from '@openmaic/lib/ai/providers';
...
export async function resolveModel(params: {
  ...
  // Resolution order: stage route > x-model > server default model > DEFAULT_MODEL.
  ...
  // to the client x-model, then the teacher-selected server default model
  // (configured in the settings page), then DEFAULT_MODEL. There is intentionally
  // no hardcoded model fallback — if nothing resolves we fail loud rather than
  // silently pick a vendor default.
  const stageRoute = getStageRoute(params.stage);
  const stageModel = stageRoute?.model;
  const serverDefault = findServerDefaultModelString();
  const modelString = stageModel || params.modelString || serverDefault || process.env.DEFAULT_MODEL;
  if (!modelString) {
    throw new Error(
      'No model could be resolved. Configure DEFAULT_MODEL (and/or a MODEL_ROUTES entry for this stage), select a default model in the teacher settings page, or send a model via x-model.',
    );
  }
