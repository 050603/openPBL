...
import type { VoiceDesign } from '@openmaic/lib/audio/voice-design';
import { USER_AVATAR } from '@openmaic/lib/types/roundtable';
import type { Participant, ParticipantRole } from '@openmaic/lib/types/roundtable';
...
export async function loadGeneratedAgentsForStage(stageId: string): Promise<string[]> {
  const { getGeneratedAgentsByStageId } = await import('@openmaic/lib/utils/database');
...
export async function saveGeneratedAgents(
  ...
  void import('@openmaic/lib/audio/agent-voice')
