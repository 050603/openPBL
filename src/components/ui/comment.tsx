  ...
  return (
    ...
    >
      <div className="relative flex items-center">
        ...
        {!locked && isMyComment && (hovering || dropdownOpen) && (
...
function CommentMoreDropdown(props: {
  ...
  const onDeleteComment = React.useCallback(() => {
    if (!comment.id) return alert('操作过于频繁，请稍后重试。');
...
export function CommentCreateForm({
  ...
  const onAiReply = usePluginOption(discussionPlugin, 'onAiReply');
  ...
  const onAddComment = React.useCallback(async () => {
    ...
    if (discussionId) {
      ...
      if (discussion.source === 'ai-proactive' && onAiReply) {
  ...
  }, [commentContent, commentValue, commentEditor.tf, discussionId, discussions, editor, onAiReply, submitting]);
  ...
  return (
    <div className={cn('flex w-full', className)}>
      ...
      <div className={cn(
        'relative flex grow gap-2',
        (submitting || replyError) && 'pb-5'
      )}>
        <Plate
          onChange={({ value }) => {
        ...
        >
          <EditorContainer variant="comment">
            ...
            <Button
              ...
              aria-label="发送批注回复"
              disabled={commentContent.trim().length === 0 || submitting}
              onClick={(e) => {
                e.stopPropagation();
                void onAddComment();
              }}
            >
              <div className="flex size-6 items-center justify-center rounded-full">
                {submitting ? <LoaderCircleIcon className="animate-spin" /> : <ArrowUpIcon />}
              </div>
            </Button>
          </EditorContainer>
        </Plate>
        {submitting ? (
          <p className="absolute top-full left-1 text-[11px] text-muted-foreground">AI 组员正在围绕这段内容思考…</p>
        ) : replyError ? (
          <p className="absolute top-full left-1 text-[11px] text-destructive">{replyError}</p>
        ) : null}
      </div>
    </div>
  );
...
export const formatCommentDate = (date: Date) => {
  ...
  if (diffMinutes < 60) {
    return diffMinutes < 1 ? '刚刚' : `${diffMinutes} 分钟`;
  }
  if (diffHours < 24) {
    return `${diffHours} 小时`;
  }
