...
import { HoverCard, HoverCardContent, HoverCardTrigger } from '@openmaic/components/ui/hover-card';
import { Progress } from '@openmaic/components/ui/progress';
...
export const ContextReasoningUsage = ({
  ...
  const { usage, modelId } = useContextValue();
  const reasoningTokens = (usage as (typeof usage & { reasoningTokens?: number }) | undefined)?.reasoningTokens ?? 0;
...
export const ContextCacheUsage = ({ className, children, ...props }: ContextCacheUsageProps) => {
  const { usage, modelId } = useContextValue();
  const cacheTokens = (usage as (typeof usage & { cachedInputTokens?: number }) | undefined)?.cachedInputTokens ?? 0;
