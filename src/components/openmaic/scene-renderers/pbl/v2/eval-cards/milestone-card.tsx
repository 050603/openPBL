...
import {
  ...
  stripEvaluationTail,
} from '@openmaic/lib/pbl/v2/operations/eval-tail-parser';
import { MarkdownText } from '../markdown-text';
import { StarRating } from './star-rating';
import type { PBLEvaluation, PBLHandover } from '@openmaic/lib/pbl/v2/types';
...
export function MilestoneCard({ evaluation, handover, onContinue, className }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Narrative paragraph */}
      {narrative && (
        <div className="relative text-sm leading-relaxed text-stone-800">
          <MarkdownText
            content={narrative}
      ...
      {/* Performance prose */}
      {performance && (
        <div className="relative border-l-2 border-violet-300 bg-white/35 py-1.5 pl-3 pr-2 text-[13px] italic text-stone-600">
          {performance}
        </div>
      )}
        ...
        {handover ? (
          <div className="space-y-2">
            <div className="text-xs text-stone-600">
              ...
              <span className="font-medium text-stone-800 ml-1">{handover.nextMilestoneTitle}</span>
              {!consumed && (
            ...
            <button
              ...
              className={cn(
                ...
                consumed
                  ? 'cursor-default border border-stone-300 bg-stone-100 text-stone-500 shadow-none'
