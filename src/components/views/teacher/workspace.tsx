...
export function WorkspaceTeacherView({
  ...
  if (!groups.length) {
    return (
      <Card className="grid place-items-center py-20 text-sm text-stone-500">
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Card>
          <div className="text-sm text-stone-500">个人项目总数</div>
          <div className="mt-2 text-2xl font-bold">{groups.length}</div>
        </Card>
        <Card>
          <div className="text-sm text-stone-500">已上传作品</div>
        ...
        <Card>
          <div className="text-sm text-stone-500">平均进度</div>
        ...
        <Card>
          <div className="text-sm text-stone-500">需介入</div>
          ...
          {proposalDiagnosis.length > 0 ? (
            <div className="grid gap-3 xl:grid-cols-2">
              {proposalDiagnosis.map((d) => (
                <div className="rounded-[8px] border border-stone-200 bg-stone-50 p-4" key={d.groupId}>
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <div className="font-bold">{d.groupName}</div>
                    <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${d.source === "llm" ? "bg-[var(--pbl-success-soft)] text-[var(--pbl-success)]" : "bg-stone-100 text-stone-600"}`}>
                      {d.source === "llm" ? "LLM" : "本地"}
                    </span>
                  </div>
                  <div className="mb-1 text-xs text-stone-500">选题：{d.topic || "未填写"}</div>
                  <p className="text-sm leading-6 text-stone-700">{d.diagnosis}</p>
                  {d.risks.length > 0 ? (
                    <div className="mt-2 text-xs leading-5 text-[var(--pbl-danger)]">风险：{d.risks.join("、")}</div>
                  ) : null}
                  <div className="mt-3">
                    <div className="mb-1 text-xs font-semibold text-stone-700">可推送的追问问题（可编辑）：</div>
                    <textarea
                      className="min-h-[60px] w-full rounded-[6px] border border-stone-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500"
              ...
              return (
                <li
                  className={`cursor-pointer rounded-[6px] border px-3 py-2 transition ${
                    g.id === activeId ? "border-blue-400 bg-[var(--pbl-teacher-soft)]/60" : "border-stone-200 bg-white hover:border-[var(--pbl-teacher-border)]"
                  }`}
                  key={g.id}
                ...
                >
                  ...
                  <div className="mt-1 truncate text-xs text-stone-500">{g.topic || "待确定选题"}</div>
          ...
          <div className="space-y-3">
            <Card>
              ...
              <div className="mt-3 flex items-center gap-3">
                ...
                <div className="text-sm text-stone-500">{active.members.map((m) => m.name).join("、")}</div>
              </div>
            </Card>
            ...
            <Card>
              ...
              {activeSignal ? (
                <div className="rounded-[8px] border border-[var(--pbl-warning-soft)] bg-[var(--pbl-warning-soft)]/50 p-4">
                  <div className="font-bold text-[var(--pbl-warning)]">{activeSignal.reasons.join("、")}</div>
                  <p className="mt-2 text-sm leading-6 text-stone-700">{activeSignal.supportCard}</p>
                  <div className="mt-2 text-xs leading-5 text-stone-500">依据：{activeSignal.evidence.join("；")}</div>
                  <PrimaryButton className="mt-3 h-9 px-3 text-sm" onClick={confirmAiSupport} type="button">
                    <Send size={15} /> 教师确认并推送
                  </PrimaryButton>
                </div>
              ) : (
                <div className="rounded-[8px] border border-dashed border-stone-300 py-7 text-center text-sm text-stone-500">
                  点击上方刷新获取 AI 观察。
                </div>
              )}
            </Card>
            ...
            <div className="grid gap-3 lg:grid-cols-2">
              <Card>
                ...
                {groupSubmission ? (
                  <div className="rounded-[6px] border border-stone-200 bg-stone-50 p-3 text-sm">
                    <p className="line-clamp-4 text-stone-700" data-testid="group-doc-preview">
                ...
                <div className="mt-3">
                  <div className="mb-1 text-xs text-stone-500">本阶段进度</div>
              ...
              <Card>
                ...
                {groupUploads.length > 0 ? (
                  <ul className="space-y-2" data-testid="group-uploads-list">
                    {groupUploads.map((u) => (
                      <li className="flex items-center gap-3 rounded-[6px] border border-stone-200 px-3 py-2" key={u.id}>
                        <FileBadge type={u.fileType} />
                        <div className="min-w-0 flex-1">
                          <div className="truncate text-sm font-semibold">{u.fileName}</div>
                          <div className="text-xs text-stone-500">{u.title} · {u.size} · {u.studentName ?? "未知"}</div>
                        </div>
                        <button
                          className="inline-flex h-8 items-center gap-1 rounded-[5px] border border-[var(--pbl-teacher-border)] px-2 text-xs font-semibold text-[var(--pbl-teacher)] hover:bg-[var(--pbl-teacher-soft)]"
                        ...
                        <a
                          className="inline-flex h-8 items-center gap-1 rounded-[5px] border border-stone-300 px-2 text-xs font-semibold text-stone-600 hover:bg-stone-50"
                          href={u.url}
                          target="_blank"
                          rel="noreferrer"
                        >
...
function UploadPreviewOverlay({ upload, onClose }: { upload: CourseUpload; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col bg-stone-900/60" role="dialog" aria-modal="true" data-testid="upload-preview-overlay">
      <div className="flex items-center justify-between bg-white px-5 py-3 shadow">
        ...
        <div className="flex items-center gap-2">
          <a
            className="inline-flex h-9 items-center gap-1 rounded-[6px] bg-[var(--pbl-teacher)] px-4 text-sm font-semibold text-white hover:bg-[var(--pbl-teacher-hover)]"
            href={upload.url}
            target="_blank"
            rel="noreferrer"
          >
          ...
          <button
            className="inline-flex h-9 items-center gap-1 rounded-[6px] border border-stone-200 bg-white px-4 text-sm font-semibold text-stone-600 hover:bg-stone-50"
            onClick={onClose}
            type="button"
          >
            <X size={16} /> 关闭
      ...
      <div className="min-h-0 flex-1 overflow-auto bg-stone-100 p-6">
        <div className="mx-auto max-w-4xl">
          {isImage ? (
            ...
            <div className="grid place-items-center rounded-[8px] bg-white py-20 shadow-lg">
              <FileText className="text-stone-400" size={48} />
              <p className="mt-4 text-sm text-stone-500">此文件类型暂不支持在线预览</p>
              <a
                className="mt-4 inline-flex h-10 items-center gap-2 rounded-[6px] bg-[var(--pbl-teacher)] px-5 text-sm font-semibold text-white hover:bg-[var(--pbl-teacher-hover)]"
                href={upload.url}
                target="_blank"
                rel="noreferrer"
              >
...
function DocumentPreviewOverlay({
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col bg-stone-900/60" role="dialog" aria-modal="true" data-testid="doc-preview-overlay">
      <div className="flex items-center justify-between bg-white px-5 py-3 shadow">
        <div className="min-w-0">
          <h3 className="truncate text-lg font-bold">{groupName} · 方案文档</h3>
          <p className="text-xs text-stone-500">更新时间：{new Date(updatedAt).toLocaleString("zh-CN")}</p>
        </div>
        <button
          className="inline-flex h-9 items-center gap-1 rounded-[6px] border border-stone-200 bg-white px-4 text-sm font-semibold text-stone-600 hover:bg-stone-50"
          onClick={onClose}
      ...
      <div className="min-h-0 flex-1 overflow-auto bg-stone-100 p-6">
        <div
          className="prose prose-stone mx-auto max-w-3xl rounded-[8px] bg-white p-8 shadow-lg"
