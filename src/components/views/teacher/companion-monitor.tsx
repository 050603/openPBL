    ...
    <Card className="mt-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">左侧按学生聚合进度与风险，右侧查看完整对话、证据并完成教师处理。</p>
        </div>
        <Pill tone={commonIssues.length ? "red" : "green"}>{commonIssues.length ? `${commonIssues.length} 个共性问题` : "暂无共性问题"}</Pill>
      </div>

      {commonIssues.length ? (
        <section className="mt-4 rounded-lg border border-rose-200 bg-rose-50/60 p-3">
          <div className="mb-2 flex items-center gap-2 text-sm font-black text-rose-800"><Users size={16} />班级共性问题</div>
          <ul className="space-y-2">{commonIssues.map((issue) => <li className="flex items-start justify-between gap-3 text-sm" key={issue.id}><span><strong>{issue.title}</strong><span className="mt-0.5 block text-slate-600">{issue.summary}</span></span><span className="shrink-0 font-bold text-rose-700">{issue.studentIds.length} 人</span></li>)}</ul>
        </section>
      ) : null}

      <div className="mt-4 grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="rounded-lg border border-slate-200 bg-slate-50/70 p-3">
          <div className="mb-2 flex items-center justify-between"><h4 className="text-sm font-black text-slate-800">学生列表</h4><span className="text-xs text-slate-400">{students.length} 人</span></div>
          <ul className="space-y-2">
            {studentRows.map(({ student, progress, signals, messages }) => (
              <li key={student.id}>
                <button aria-label={`${student.name}${signals.length ? " 有干预信号" : ""}`} className={cn("w-full rounded-lg border p-2.5 text-left transition", selectedStudent?.id === student.id ? "border-indigo-300 bg-white shadow-sm" : "border-transparent hover:border-slate-200 hover:bg-white")} onClick={() => setSelectedStudentId(student.id)} type="button">
                  <div className="flex items-center gap-2.5"><span className="relative"><Avatar name={student.name} size={32} />{signals.length ? <CircleAlert aria-label="有干预信号" className="absolute -right-2 -top-2 fill-white text-rose-600" size={17} /> : null}</span><span className="min-w-0 flex-1"><strong className="block truncate text-sm">{student.name}</strong><span className={cn("text-[11px]", signals.length ? "font-bold text-rose-700" : "text-slate-500")}>{signals.length ? `${signals.length} 条待处理` : "暂无待处理风险"}</span></span></div>
                  <div className="mt-2"><div className="mb-1 flex justify-between text-[11px] text-slate-500"><span>阶段进度</span><strong>{progress}%</strong></div><ProgressBar className="h-1.5" tone={signals.length ? "red" : progress >= 80 ? "green" : "slate"} value={progress} /></div>
                  <div className="mt-1 text-[11px] text-slate-400">{messages} 条伴学消息</div>
                </button>
              </li>
            ))}
          </ul>
        ...
        <section className="min-w-0 rounded-lg border border-slate-200 bg-white p-4">
          {selectedStudent ? (
            <>
              <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 pb-3"><div className="flex items-center gap-3"><Avatar name={selectedStudent.name} size={42} /><div><h4 className="text-lg font-black">{selectedStudent.name}</h4><p className="text-xs text-slate-500">阶段进度 {getStudentProgress(course, stageKey, selectedStudent.id)}% · {selectedSignals.length ? `${selectedSignals.length} 条待处理信号` : "当前无待处理信号"}</p></div></div>{selectedSignals.length ? <Pill tone="red"><CircleAlert size={13} />需要教师处理</Pill> : <Pill tone="green">已处理</Pill>}</div>
              <section className="mt-4"><h5 className="flex items-center gap-2 text-sm font-black"><CircleAlert size={15} />介入详情</h5>{selectedSignals.length ? <ul className="mt-2 space-y-2">{selectedSignals.map((signal) => <li className="rounded-lg border border-rose-200 bg-rose-50/60 p-3" key={signal.id}><div className="flex items-center justify-between gap-2"><strong className="text-sm text-rose-800">{signal.title}</strong><span className="text-[11px] font-bold text-rose-700">{"severity" in signal ? signal.severity : signal.confidence === "high" ? "高置信" : "需核查"}</span></div><p className="mt-1 text-sm leading-6 text-slate-600">{"summary" in signal ? signal.summary : signal.whatHappened}</p><p className="mt-1 text-xs text-slate-400">依据：{"evidenceEventIds" in signal ? `${signal.evidenceEventIds.length} 条学习事件` : signal.evidence.join("；")}</p></li>)}</ul> : <p className="mt-2 rounded-lg border border-dashed border-slate-200 py-6 text-center text-sm text-slate-500">暂无未处理风险。教师处理完成后，学生卡片上的叹号会消失。</p>}</section>

              <section className="mt-5"><h5 className="flex items-center gap-2 text-sm font-black"><MessageSquareText size={15} />AI 对话过程</h5>{selectedMessages.length ? <ol className="mt-2 max-h-64 space-y-2 overflow-y-auto">{selectedMessages.map((message) => <li className="border-l-2 border-slate-200 pl-3" key={message.id}><div className="flex justify-between gap-3 text-[11px] text-slate-400"><span>{message.authorName ?? message.companionId ?? message.role}{message.visibility === "teacher-only" ? " · 仅教师" : ""}</span><time>{new Date(message.createdAt).toLocaleString("zh-CN")}</time></div><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-slate-700">{message.content}</p></li>)}</ol> : <p className="mt-2 text-sm text-slate-500">暂无 AI 对话记录</p>}</section>

              {selectedSignals.length ? <section className="mt-5 rounded-lg border border-indigo-100 bg-indigo-50/50 p-3"><div className="flex items-center gap-2 text-sm font-black text-indigo-900"><ClipboardCheck size={15} />教师处理</div><textarea aria-label="教师处理备注" className="mt-2 min-h-20 w-full rounded-md border border-indigo-200 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-500" onChange={(event) => setHandlingNote(event.target.value)} placeholder="记录现场介入的关键动作或给学生的下一步要求（可选）" value={handlingNote} /><PrimaryButton className="mt-2" disabled={handling} onClick={handleSelectedStudent} type="button"><ClipboardCheck size={15} />{handling ? "保存中..." : "教师已处理，消除叹号"}</PrimaryButton></section> : null}

              <section className="mt-5"><h5 className="flex items-center gap-2 text-sm font-black"><Target size={15} />教师持续目标</h5>{selectedDirectives.length ? <ul className="mt-2 space-y-2">{selectedDirectives.map((directive) => <li className="rounded-lg border border-indigo-200 bg-indigo-50 p-3" key={directive.id}><div className="flex justify-between gap-3"><strong className="text-sm">{directive.goal}</strong><span className="text-xs font-bold text-indigo-700">{directive.status}</span></div><p className="mt-1 text-xs leading-5 text-slate-600">{directive.instruction}</p>{directive.status === "active" ? <PrimaryButton className="mt-2" onClick={() => session.upsertTeacherAgentDirective({ ...directive, status: "revoked", revokedAt: new Date().toISOString() })} type="button" variant="outline"><XCircle size={14} />撤销</PrimaryButton> : null}</li>)}</ul> : null}<TeacherDirectiveForm course={course} initialStudentId={selectedStudent.id} stageKey={stageKey} /></section>
            </>
          ) : <p className="py-12 text-center text-sm text-slate-500">暂无学生数据</p>}
        </section>
      </div>
    </Card>
  );
