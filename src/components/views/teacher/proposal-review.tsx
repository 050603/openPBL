...
import { CheckCircle2, Eye, MessageSquareWarning, UserRound } from "lucide-react";
...
export function ProposalReviewTeacherView({ course, onSelectGroup }: { course: Course; onSelectGroup?: (groupId: string) => void }) {
  ...
  return <div className="space-y-6"><header className="border-b border-[var(--pbl-border)] pb-5"><p className="text-sm font-semibold text-[var(--pbl-teacher)]">方案构思与校准</p><h2 className="font-editorial mt-1 text-2xl font-semibold">逐个校准个人项目方向，学生保留最终项目所有权</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--pbl-text-muted)]">AI 负责多角度建议和风险扫描；教师检查目标、边界与可行性，并确认学生能够解释自己的选择。</p></header>
    ...
    <div className="grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)]"><Card><h3 className="mb-3 flex items-center gap-2 font-bold"><UserRound size={18} />学生个人项目</h3><div className="space-y-2">{projects.map(({ student, project }) => <button className={`w-full rounded-[8px] border p-3 text-left ${student.id === active?.student.id ? "border-blue-400 bg-blue-50" : "border-slate-200 hover:border-blue-200"}`} key={student.id} onClick={() => setActiveStudentId(student.id)} type="button"><div className="flex items-center justify-between gap-2"><span className="font-semibold">{student.name}</span><Pill tone={project?.teacherApproval?.status === "approved" ? "green" : project?.teacherApproval?.status === "revision" ? "orange" : project?.proposal ? "blue" : "gray"}>{project?.teacherApproval?.status === "approved" ? "已确认" : project?.teacherApproval?.status === "revision" ? "需修订" : project?.proposal ? "待校准" : "未提交"}</Pill></div><p className="mt-1 truncate text-xs text-slate-500">{project?.proposal?.projectQuestion || "尚未填写项目问题"}</p></button>)}</div></Card>
