...
import { Card } from '@openmaic/components/ui/card';
import { Tooltip, TooltipContent, TooltipTrigger } from '@openmaic/components/ui/tooltip';
import { OutlinesEditor } from '@openmaic/components/generation/outlines-editor';
...
import { useVoxCPMVoiceProfiles } from '@openmaic/lib/audio/voxcpm-voices';
import { useI18n } from '@openmaic/lib/hooks/use-i18n';
import {
  fetchSceneActions,
  fetchSceneContent,
  generateAndStoreTTS,
} from '@openmaic/lib/hooks/use-scene-generator';
...
import {
  ...
  cleanupOldImages,
  storeImages,
} from '@openmaic/lib/utils/image-storage';
...
import type { SceneOutline, PdfImage, ImageMapping } from '@openmaic/lib/types/generation';
import { AgentRevealModal } from '@openmaic/components/agent/agent-reveal-modal';
...
function GenerationPreviewContent() {
    ...
    try {
      ...
      if (settings.agentMode === 'auto') {
        ...
        try {
          ...
          const { saveGeneratedAgents } = await import('@openmaic/lib/orchestration/registry/store');
  ...
  if (!sessionLoaded) {
    return (
      <div className="min-h-[100dvh] w-full bg-gradient-to-b from-stone-50 to-stone-100 dark:from-slate-950 dark:to-slate-900 flex items-center justify-center p-4">
  ...
  if (isReviewingOutlines) {
    ...
    return (
      <div className="min-h-[100dvh] w-full bg-gradient-to-b from-stone-50 to-stone-100 dark:from-slate-950 dark:to-slate-900 flex flex-col items-center p-4 relative overflow-hidden">
        <motion.div
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-gradient-to-b from-stone-50 to-stone-100 dark:from-slate-950 dark:to-slate-900 flex flex-col items-center justify-center p-4 relative overflow-hidden text-center">
      {/* Background Decor */}
      ...
      <div className="z-10 w-full max-w-lg space-y-8 flex flex-col items-center">
        <motion.div
          ...
          className="w-full"
        >
          <Card className="relative overflow-hidden border-muted/40 shadow-2xl bg-white/80 dark:bg-stone-900/80 backdrop-blur-xl min-h-[400px] flex flex-col items-center justify-center p-8 md:p-12">
            {/* Progress Dots */}
...
export default function GenerationPreviewPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-[100dvh] w-full bg-gradient-to-b from-stone-50 to-stone-100 dark:from-slate-950 dark:to-slate-900 flex items-center justify-center">
