Create one independent horizontal animation strip for 伴学小灵, action `waiting_user`.

Input images:
- references/standard-view-01.png — standard-view reference
- canonical/canonical-base.png — canonical-base identity lock
- references/layout-guides/waiting_user.png — layout-only guide for 4 slots; never copy visible guide pixels

Action semantic: 期待地等待用户回应。
Required frame count: 4
Layout reference: references/layout-guides/waiting_user.png
...
Forbidden: ordinary idle, text, speech bubble; white or colored background residue, checkerboard, scene, floor, shadow, projection, glow, speed line, dust, motion blur, detached symbol, text, label, frame number, guide line, grid, cropped ear/sprout/scarf/limb, replacement eyes, or another character.
